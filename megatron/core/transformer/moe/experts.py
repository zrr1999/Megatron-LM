    ...
    def forward(
        ...
        if self.num_local_experts == 1:
            ...
            for _eidx, (expert, tokens, probs) in enumerate(zip(self.local_experts, tokens_list, probs_list)):
                ...
                if _w_dump and int(_layer) == 4:
                    import hashlib as _hw
                    import torch.distributed as _tdw

                    _rank = _tdw.get_rank() if _tdw.is_initialized() else 0
                    if not hasattr(self, "_e680_w_dumped"):
                        self._e680_w_dumped = set()
                    os.makedirs(_w_dump, exist_ok=True)
                    for _name, _mod in (
                        ("w1", getattr(expert, "linear_fc1", None)),
                        ("w2", getattr(expert, "linear_fc2", None)),
                    ):
                        _wt = getattr(_mod, "weight", None) if _mod is not None else None
                        _key = (_name, int(_layer), int(_eidx), int(_rank))
                        if _wt is None or _key in self._e680_w_dumped:
                            continue
                        self._e680_w_dumped.add(_key)
                        _arr = _wt.detach().contiguous().view(torch.uint16).cpu().numpy()
                        _path = os.path.join(
                            _w_dump, f"torch_{_name}_e{_eidx}_l{_layer}_r{_rank}.u16.bin"
                        )
                        _arr.tofile(_path)
                        print(
                ...
                _out_dump = _gemm_dump or os.environ.get("MODEL_REPRO_O2_DUMP_DIR")
                if _out_dump:
                    ...
                    _key = ("expert_out", int(_layer) if _layer is not None else -1, int(_eidx), int(_sek))
                    if _key not in self._e678_dumped:
                        self._e678_dumped.add(_key)
                        os.makedirs(_out_dump, exist_ok=True)
                        _arr = output.detach().float().cpu().numpy()
                        _path = os.path.join(
                            _out_dump,
                            f"torch_expert_out_e{_eidx}_l{_layer}_r{_sek}.f32.bin",
                        )
                        _arr.tofile(_path)
                        print(
