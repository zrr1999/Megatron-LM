...
def finalize_model_grads(
    ...
    # E-172: mcore's standard 1/num_tokens gradient scaling is KEPT under
    # ``use_accuracy_compatible``. An earlier alignment attempt skipped it, on the
    # theory that PaddleFleet's fixed-loss path already divides by the local valid
    # token count inside the autograd graph and that scaling again here would add a
    # global/local factor. Measured against the paddle receipts that is wrong: with
    # ``calculate_per_token_loss=true`` mcore backpropagates the raw sum(loss*mask),
    # so skipping this scaling leaves EVERY gradient num_tokens times too large
    # (measured 44.0x at step 1, whose supervised token count is exactly 44; the
    # reported grad_norm went 2512.27 -> 53.70 when the scaling was restored, and
    # the per-parameter |grad| ratio against paddle went ~44 -> ~1.008 median).
    # num_tokens here is already all-reduced across DP, which is the same quantity
    # paddle divides by, so the two sides agree; a separate 1/dp_size average would
    # double-count it.
    ...
    # All-reduce the fp32 gate wgrad that RouterGatingLinearFunction records on
    # ``param._run_torch_gate_fp32_wgrad`` across DP, matching the reference
    # implementation. A no-op when that mechanism is not enabled.
